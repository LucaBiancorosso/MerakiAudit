...
def load_standards(path: Path) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for row in rows[1:]:
        ...
        ssid_name = str(row_dict.get("ssid_name") or "").strip()

        # Skip blank rows or the colour-hint row (row 2 in template)
